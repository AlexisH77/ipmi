...
void draw() {
  ...
  if (frameCount <= 400) {
    ...
    float destinoX = 0;
    if (foto2PosX > destinoX) {
      foto2PosX -= 5;
      if (foto2PosX < destinoX) foto2PosX = destinoX;
    }
    image(foto2, foto2PosX, foto2PosY, width, height);
    ...
    if (foto3PosY > 0) {
      ...
      if (foto3PosY <= 0) {
        foto3PosY = 0;
        foto3EnPosicion = true; 
      }
    }
    image(foto3, foto3PosX, foto3PosY, width, height);
...
void mousePressed() {
  if (frameCount > 1000 && foto3EnPosicion) {
